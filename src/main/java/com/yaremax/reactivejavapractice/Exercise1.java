...
//        StreamSources.intNumbersStream().filter(x -> x > 5).limit(3).skip(1).forEach(System.out::println);
...
//        StreamSources.userStream().map(user -> user.getFirstName()).forEach(System.out::println);
        ...
        StreamSources.userStream()
                .filter(user -> StreamSources.intNumbersStream().anyMatch(n -> user.getId() == n))
